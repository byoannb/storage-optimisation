...
# Fonction pour appliquer la stratégie First-Fit Decreasing (FFD)
ffd <- function(jeux, m) {
  n <- length(jeux)
  stockages <- numeric(n)
  num_stockages <- 0
  for (i in 1:n) {
    for (j in 1:num_stockages) {
      if (jeux[i] + stockages[j] <= m) {
        stockages[j] <- stockages[j] + jeux[i]
        break
      }
    }
    if (j > num_stockages) {
      num_stockages <- num_stockages + 1
      stockages[num_stockages] <- jeux[i]
    }
  }
